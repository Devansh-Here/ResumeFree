...
// ── Section Heading (left bar accent, emerald) ─────────────────
function SectionHeading({ title }) {
  return (
    <div className="flex items-center gap-2 mt-4 mb-2">
      <div className="w-[3px] h-3 bg-[#059669] rounded-full flex-shrink-0" />
      <span
...
export default function ModernTemplate({ resume }) {
  ...
  return (
    ...
    >
      {/* ── Header (left-aligned, emerald underline) ── */}
      <div className="mb-4 pb-3 border-b-2 border-[#059669]">
        <h1
          className="font-bold text-[24px] tracking-tight text-[#0a1628]"
          style={{ fontFamily: "'Inter', sans-serif" }}
        >
          {personal.name || <EmptyField text="Your Name" />}
        </h1>

        <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1.5 text-[10px] text-[#1e3a5f]">
        ...
        {(personal.linkedin || personal.github || personal.portfolio) && (
          <div className="flex flex-wrap gap-x-3 mt-1 text-[10px] font-medium text-[#059669]">
            {personal.linkedin && (
      ...
      {/* ── Experience (before Education — Modern prioritizes work) ── */}
      {hasExperience && (
        <>
                    ...
                    {exp.bullets
                      ...
                      .map((bullet, i) => (
                        <li key={i} className="flex gap-1.5 text-[#1e3a5f]">
                          <span className="flex-shrink-0 mt-0.5 text-[#059669]">
                            ▸
                          </span>
                          <span>{bullet}</span>
                        </li>
            ...
            {education.map((edu) => (
              <div key={edu.id}>
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="text-right flex-shrink-0 text-[#4a6fa5]">
      ...
      {/* ── Skills (emerald tag pills) ── */}
      {hasSkills && (
        <>
          <SectionHeading title="Skills" />
          <div className="flex flex-wrap gap-1.5">
            {[...skills.technical, ...skills.tools, ...skills.languages].map(
              (skill, i) => (
                <span
                  key={i}
                  className="px-2 py-0.5 bg-[#ecfdf5] text-[#059669] rounded-full text-[10px] font-medium"
                >
                  {skill}
                </span>
              )
